...
var require_semver = __commonJS({
  "../../node_modules/.pnpm/semver@5.7.2/node_modules/semver/semver.js"(exports, module2) {
    ...
    function SemVer(version2, options) {
      ...
      debug17("SemVer", version2, options);
    ...
    SemVer.prototype.compare = function(other) {
      debug17("SemVer.compare", this.version, this.options, other);
    ...
    SemVer.prototype.comparePre = function(other) {
      ...
      do {
        ...
        debug17("prerelease compare", i2, a, b);
    ...
    function Comparator(comp, options) {
      ...
      debug17("comparator", comp, options);
      this.options = options;
      this.loose = !!options.loose;
      this.parse(comp);
      if (this.semver === ANY) {
    ...
    Comparator.prototype.test = function(version2) {
      debug17("Comparator.test", version2, this.options.loose);
      if (this.semver === ANY) {
        return true;
      }
      if (typeof version2 === "string") {
    ...
    Range.prototype.parseRange = function(range) {
      ...
      debug17("hyphen replace", range);
      range = range.replace(safeRe[COMPARATORTRIM], comparatorTrimReplace);
      debug17("comparator trim", range, safeRe[COMPARATORTRIM]);
    ...
    function replaceTilde(comp, options) {
      ...
      return comp.replace(r, function(_, M, m, p, pr) {
        ...
        if (isX(M)) {
          ...
          debug17("replaceTilde pr", pr);
          ret = ">=" + M + "." + m + "." + p + "-" + pr + " <" + M + "." + (+m + 1) + ".0";
        } else {
          ret = ">=" + M + "." + m + "." + p + " <" + M + "." + (+m + 1) + ".0";
        }
        debug17("tilde return", ret);
        return ret;
      });
    }
    ...
    function replaceCaret(comp, options) {
      debug17("caret", comp, options);
      var r = options.loose ? safeRe[CARETLOOSE] : safeRe[CARET];
      return comp.replace(r, function(_, M, m, p, pr) {
        debug17("caret", comp, _, M, m, p, pr);
        var ret;
        if (isX(M)) {
          ret = "";
        } else if (isX(m)) {
        ...
        debug17("caret return", ret);
        return ret;
      });
    }
    function replaceXRanges(comp, options) {
      debug17("replaceXRanges", comp, options);
    ...
    function replaceXRange(comp, options) {
      ...
      return comp.replace(r, function(ret, gtlt, M, m, p, pr) {
        debug17("xRange", comp, ret, gtlt, M, m, p, pr);
        var xM = isX(M);
        var xm = xM || isX(m);
        var xp = xm || isX(p);
        var anyX = xp;
        ...
        debug17("xRange return", ret);
        return ret;
      });
    }
    function replaceStars(comp, options) {
      debug17("replaceStars", comp, options);
...
var require_lib = __commonJS({
  "../../node_modules/.pnpm/which@3.0.0/node_modules/which/lib/index.js"(exports, module2) {
    var isexe = require_isexe();
    var { join: join20, delimiter: delimiter2, sep: sep3, posix } = require("path");
    ...
    var getPathPart = (raw, cmd) => {
      ...
      return prefix + join20(pathPart, cmd);
    };
...
var require_semver2 = __commonJS({
  "../../node_modules/.pnpm/semver@6.3.1/node_modules/semver/semver.js"(exports, module2) {
    ...
    Range.prototype.parseRange = function(range) {
      ...
      debug17("comparator trim", range, safeRe[t.COMPARATORTRIM]);
...
async function collectAppPrefixBytecodeFiles({
  ...
  for (const bundlePath of Object.keys(appFiles)) {
    ...
    const stagedFsPath = deriveStagedPycFsPath(
      stagingDir,
      (0, import_path17.join)(workPath, bundlePath.replaceAll("/", import_path17.sep)),
      pythonMajor,
      pythonMinor
    );
  ...
  for (const result of results) {
    if (!result)
      continue;
    files[result.bundlePath] = new import_build_utils20.FileFsRef({
...
var InstalledPythonDistributions = class _InstalledPythonDistributions {
    ...
    for (const { packageName, sitePackagesDir, files } of distributionGroups) {
      for (const { relativePath } of files) {
        const pycRelativePath = derivePycPath(
          relativePath.replaceAll(import_path18.sep, "/"),
          this.pythonMajor,
          this.pythonMinor
        );
        if (!pycRelativePath)
  ...
  async collectExistingBytecode(pending) {
    ...
    for (const result of results) {
      if (!result)
        continue;
      files[result.bundlePath] = new import_build_utils21.FileFsRef({
        fsPath: result.srcFsPath,
        size: result.size
      });
      totalSize += result.size;
...
async function getDevSidecars({
  ...
  const isPyprojectEntrypoint = (0, import_path20.basename)(build2.src ?? "") === "pyproject.toml";
  const isPyprojectService = service !== void 0 && (0, import_path20.basename)(service.entrypoint ?? "") === "pyproject.toml";
  if (build2.config?.middleware === true || service !== void 0 && !isPyprojectService || service === void 0 && !isPyprojectEntrypoint && (typeof framework !== "string" || !(0, import_build_utils23.isPythonFramework)(framework))) {
    return [];
  }
...
var frameworkHooks = {
  django: async ({
    ...
    if (baseDir === void 0) {
      if (!import_fs20.default.existsSync((0, import_path20.join)(workPath, "manage.py"))) {
        (0, import_build_utils23.debug)("Django hook: no manage.py detected, skipping");
        return;
      }
      baseDir = "";
    }
    const djangoPath = (0, import_path20.join)(workPath, baseDir);
    let settingsResult;
    try {
...
Hint: activate a venv or run with \`uv run vercel dev\``;
      ...
      throw new import_build_utils23.NowBuildError({
        code: "DJANGO_SETTINGS_FAILED",
        message: `Failed to read Django application settings from ${djangoPath}/manage.py:
${detail}`
      });
    }
    (0, import_build_utils23.debug)(`Django settings: ${JSON.stringify(settingsResult)}`);
    ...
    if (typeof asgiApp === "string") {
      ...
      (0, import_build_utils23.debug)(`Django hook: ASGI entrypoint: ${ep} (variable: ${variableName})`);
    ...
    if (workPath && venvPath) {
      const outputStaticDir = (0, import_path20.join)(workPath, ".vercel", "output", "static");
    ...
    return {
      ...
      djangoStatic,
      extraPythonPath: baseDir ? (0, import_path20.join)(workPath, baseDir) : void 0
    };
  },
  fastapi: async ({
    pythonEnv,
    ...
    const entrypointAbs = (0, import_path20.join)(workPath, entrypointRel);
    const outputStaticDir = (0, import_path20.join)(workPath, ".vercel", "output", "static");
...
async function downloadFilesInWorkPath({
  ...
  (0, import_build_utils23.debug)("Downloading user files...");
  let downloadedFiles = await (0, import_build_utils23.download)(files, workPath, meta);
  if (meta.isDev && entrypoint) {
    ...
    const { devCacheDir = (0, import_path20.join)(workPath, ".now", "cache") } = meta;
    const cacheKey = (0, import_path20.basename)(entrypoint).replace(/\./g, "_");
    const destCache = (0, import_path20.join)(devCacheDir, cacheKey);
    await (0, import_build_utils23.download)(downloadedFiles, destCache);
    downloadedFiles = await (0, import_build_utils23.glob)("**", destCache);
    workPath = destCache;
  }
  return workPath;
}
...
async function getPythonLambdaOptions({
  ...
  for (const sourceFile of sources) {
    const lambdaOptions = await (0, import_build_utils23.getLambdaOptionsFromFunction)({
      sourceFile,
      config
    });
...
async function installInjectedPackage({
  ...
  const localDir = (0, import_path20.join)(__dirname, "..", "..", "..", "python", name);
  const isLocalDev = allowLocalSource && import_fs20.default.existsSync((0, import_path20.join)(localDir, "pyproject.toml"));
  const dep = envOverride || (isLocalDev ? localDir : requirement);
  const noExclude = ["--exclude-newer-package", `${name}=false`];
  (0, import_build_utils23.debug)(`Installing ${dep}`);
  await uv.pip({
    venvPath,
    projectDir,
    args: [
...
var build = async ({
  ...
  const isPyprojectEntrypoint = entrypoint !== void 0 && (0, import_path20.basename)(entrypoint) === "pyproject.toml";
  if (isPyprojectEntrypoint && entrypoint !== "pyproject.toml") {
    throw new import_build_utils23.NowBuildError({
      code: "PYTHON_INVALID_PYPROJECT_ENTRYPOINT",
      message: `A "pyproject.toml" entrypoint must sit at the service root. Set the service "root" to "${(0, import_path20.dirname)(entrypoint)}" and use entrypoint "pyproject.toml".`
    });
  }
  const builderSpan = parentSpan ?? new import_build_utils23.Span({ name: "vc.builder" });
  const framework = config?.framework;
  ...
  (0, import_build_utils23.debug)(`workPath: ${workPath}`);
  ...
  if (isPyprojectEntrypoint || !service && (0, import_build_utils23.isPythonFramework)(framework)) {
  ...
  try {
    if (meta.isDev) {
      const setupCfg = (0, import_path20.join)(workPath, "setup.cfg");
  ...
  if (isPyprojectEntrypoint) {
    ...
    detected = await detectPythonEntrypoint(
      ...
      entrypoint ? {
        ...
        varName: service && (0, import_build_utils23.isScheduleTriggeredService)(service) ? void 0 : handlerFunction
      } : void 0,
      service,
      repoRootPath
    ) ?? void 0;
  ...
  const entryDirectory = detected?.baseDir ?? (entrypoint ? (0, import_path20.dirname)(entrypoint) : ".");
  const entrypointAbsDir = (0, import_path20.join)(workPath, entryDirectory);
...
`);
  ...
  const venvPath = service?.name ? (0, import_path20.join)(workPath, ".vercel", "python", "services", service.name, ".venv") : (0, import_path20.join)(workPath, ".vercel", "python", ".venv");
  const hasCachedVenv = import_fs20.default.existsSync((0, import_path20.join)(venvPath, "pyvenv.cfg"));
  ...
  if ((0, import_build_utils23.isPythonFramework)(framework)) {
    const {
      cliType,
      lockfileVersion,
      packageJsonPackageManager,
      turboSupportsCorepackHome
    } = await (0, import_build_utils23.scanParentDirs)(workPath, true);
    spawnEnv = (0, import_build_utils23.getEnvForPackageManager)({
      cliType,
      lockfileVersion,
      packageJsonPackageManager,
      env: process.env,
  ...
  await builderSpan.child(import_build_utils23.BUILDER_INSTALLER_STEP, {
    ...
    if (projectInstallCommand) {
      ...
      await (0, import_build_utils23.execCommand)(projectInstallCommand, {
        env: pythonEnv,
        cwd: workPath
      });
    ...
    if (!assumeDepsInstalled) {
      ...
      const cachedLockPath = (0, import_path20.join)(uvCacheDir, lockCacheKey);
  ...
  if ((0, import_build_utils23.isPythonFramework)(framework)) {
    ...
    config?.buildCommand;
    await builderSpan.child(import_build_utils23.BUILDER_COMPILE_STEP, {
      buildCommand: projectBuildCommand || void 0
    }).trace(async () => {
      if (projectBuildCommand) {
        console.log(`Running "${projectBuildCommand}"`);
        await (0, import_build_utils23.execCommand)(projectBuildCommand, {
          env: pythonEnv,
          cwd: workPath
        });
      } else {
  ...
  await installInjectedPackage({
    ...
    uv,
    venvPath,
    projectDir: (0, import_path20.join)(workPath, entryDirectory),
    pipPlatformArgs
  });
  ...
  const writeGeneratedQueueHandler = async (outputPath, declaration) => {
    ...
    await import_fs20.default.promises.mkdir((0, import_path20.dirname)((0, import_path20.join)(workPath, generatedPath)), {
      recursive: true
    });
    await import_fs20.default.promises.writeFile(
      (0, import_path20.join)(workPath, generatedPath),
  ...
  if (registerPreDeploy && typeof preDeployCommand === "string") {
    ...
    registerPreDeploy(async () => {
      await builderSpan.child(import_build_utils23.BUILDER_PRE_DEPLOY_STEP, {
        preDeployCommand
      }).trace(async () => {
        console.log(`Running pre-deploy command: \`${preDeployCommand}\``);
        await (0, import_build_utils23.execCommand)(preDeployCommand, {
          env: capturedEnv,
          cwd: capturedCwd
        });
      });
  ...
  if (entrypoint) {
    (0, import_build_utils23.debug)("Entrypoint is", entrypoint);
    const moduleName = entrypointToModule(entrypoint);
    if (handlerFunction) {
      const entrypointPath = (0, import_path20.join)(workPath, entrypoint);
    ...
    (0, import_build_utils23.debug)("Entrypoint with suffix is", entrypointWithSuffix);
  ...
  const files = await (0, import_build_utils23.glob)("**", globOptions);
  const appPythonSourceFiles = Object.keys(files).filter((file) => file.endsWith(".py")).map((file) => (0, import_path20.join)(workPath, file)).sort();
  if (djangoStatic?.manifestRelPath) {
    files[djangoStatic.manifestRelPath] = new import_build_utils23.FileFsRef({
      fsPath: (0, import_path20.join)(workPath, djangoStatic.manifestRelPath)
    });
  }
  const handlerPyFilename = "vc__handler__python";
  if (config.framework === "fasthtml") {
    const { SESSKEY = "" } = process.env;
    files[".sesskey"] = new import_build_utils23.FileBlob({ data: `"${SESSKEY}"` });
  }
  await builderSpan.child("vc.builder.python.bundle").trace(async (bundleSpan) => {
    ...
    const runCompileAllAndFillBytecode = async (capacityBytes, vendorPackageTiers) => {
      try {
        ...
        (0, import_build_utils23.debug)(`bytecode precompilation error details: ${err}`);
      }
    };
    const runPrefixCompileAndFill = async (bundleResult) => {
      ...
      try {
        ...
        const stagingDir = (0, import_path20.join)(workPath, ".vercel", "python", "pycache");
  ...
  if (entrypoint && runtimeTrampoline) {
    const webFiles = {
      ...files,
      [`${handlerPyFilename}.py`]: new import_build_utils23.FileBlob({ data: runtimeTrampoline })
    };
    const lambdaOptions = await getPythonLambdaOptions({
      config,
      entrypoint
    });
    output = new import_build_utils23.Lambda({
      files: webFiles,
  ...
  for (const subscriber of subscribers) {
    ...
    subscriberLambdas[outputPath] = new import_build_utils23.Lambda({
      files: {
        ...files,
        [`${handlerPyFilename}.py`]: new import_build_utils23.FileBlob({
  ...
  for (const workflow of workflows) {
    ...
    if (workflowMode === "queue") {
      ...
      workflowLambdas[outputPath] = new import_build_utils23.Lambda({
        files: {
          ...files,
          [`${handlerPyFilename}.py`]: new import_build_utils23.FileBlob({
            data: createRuntimeTrampoline({
  ...
  if (uvLockPath && !meta.isDev) {
    try {
      await generateProjectManifest({
        ...
        uvLockPath,
        framework,
        serviceType: service ? (0, import_build_utils23.getReportedServiceType)(service) : void 0
      });
    } catch (err) {
      (0, import_build_utils23.debug)(
  ...
  if (!(0, import_build_utils23.isPythonFramework)(framework) && !service?.name && !isPyprojectEntrypoint) {
  ...
  const staticFiles = cdnOutputDir ? await (0, import_build_utils23.glob)("**", { cwd: cdnOutputDir }) : {};
...
var prepareCache = async ({
  ...
  return (0, import_build_utils23.glob)("**/.vercel/python/{.venv,services/*/.venv,cache/uv}/**", {
    cwd: root,
    ignore
  });
};
...
var defaultShouldServe = ({
  ...
  const { dir, name } = (0, import_path20.parse)(entrypoint);
