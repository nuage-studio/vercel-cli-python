...
var SWITCH_GIT_REMOTE = Symbol("switch-git-remote");
...
async function inputProject(client, org, detectedProjectName, autoConfirm = false, skipAutoDetect = false, showProjectSuggestions = false, allowTeamSelectionBack = false, repoMatches = [], gitRemoteOptions) {
    ...
    for (; ; ) {
      ...
      if (canSwitchGitRemote) {
        ...
        choices.push({
          ...
          description: current ? `Suggestions are from ${current}` : "Search projects linked to a different remote"
        });
      }
...
async function searchProjectsByRepoRoot({
  ...
  const empty = { matches: [], remoteNames: [] };
  const rootPath = await findRepoRoot(cwd);
  if (!rootPath) {
    return empty;
  }
  const gitConfigPath = getGitConfigPath({ cwd: rootPath }) ?? join2(rootPath, ".git/config");
...
async function setupAndLink(client, path2, {
  ...
  if (forceDelete) {
    ...
    (0, import_fs_extra2.remove)(join3(vercelDir, VERCEL_DIR_README));
    (0, import_fs_extra2.remove)(join3(vercelDir, VERCEL_DIR_PROJECT));
  }
  output_manager_default.print("\n");
  printAlignedLabel("Directory", humanizePath(path2));
  output_manager_default.print("\n");
  ...
  for (; ; ) {
    ...
    if (projectOrNewProjectName === BACK_TO_TEAM_SELECTION) {
      org = void 0;
      selectedGitRemoteName = void 0;
      continue;
    }
    if (projectOrNewProjectName === SWITCH_GIT_REMOTE) {
      const rootPath = await findRepoRoot(path2);
      if (!rootPath) {
        continue;
      }
      try {
        ...
        output_manager_default.debug(`Failed to switch Git remote: ${err}`);
      }
      continue;
    }
    break;
    ...
    if (rootServicesSetup.hasConfiguredServices) {
      ...
      if (rootInferredServicesChoice?.type === "single-app") {
        ...
        if (shouldPromptRoot) {
          ...
          if (rootDirectory && !await validateRootDirectory(path2, join3(path2, rootDirectory))) {
            return {
              status: "error",
              exitCode: 1,
      ...
      pathWithRootDirectory = rootDirectory ? join3(path2, rootDirectory) : path2;
...
async function resolveGitConnectIntent(client, path2, autoConfirm) {
  try {
    const gitConfig = await parseGitConfig(join3(path2, ".git/config"));
    if (!gitConfig) {
      return null;
    }
    ...
    const shouldConnect = skipPrompts || await client.input.confirm(`Connect detected Git repository?`, false);
    if (!shouldConnect) {
      return null;
    }
    const repoInfo = skipPrompts ? parseRepoUrl(remoteUrls.origin ?? Object.values(remoteUrls)[0]) : await selectAndParseRemoteUrl(client, remoteUrls);
    if (!repoInfo) {
      return null;
    }
    return { repoInfo };
    ...
    output_manager_default.debug(`Failed to detect git repository: ${error}`);
    return null;
  }
}
async function applyGitConnectIntent(client, intent, project, autoConfirm, org) {
  if (!intent) {
    return;
  }
