...
async function merge(source, destination, ignoreFilter, sourceRoot) {
  ...
  if ((0, import_error_utils.isErrnoException)(destStat)) {
    if (destStat.code === "ENOENT") {
      await (0, import_fs_extra.move)(source, destination);
      return;
    }
    throw destStat;
    ...
    if (isContainerImage(output)) {
      ...
      if (fallback) {
        ...
        if ("fsPath" in fallback) {
          try {
            await import_fs_extra2.default.link(fallback.fsPath, fallbackPath);
            usedHardLink = true;
          } catch (_) {
          }
        }
        if (!usedHardLink) {
          ...
          await (0, import_promisepipe2.default)(
            stream,
            import_fs_extra2.default.createWriteStream(fallbackPath, { mode: fallback.mode })
          );
        }
      ...
      await import_fs_extra2.default.writeJSON(prerenderConfigPath, prerenderConfig, { spaces: 2 });
  ...
  if (isBackendBuilder(build) || build.use === "@vercel/python") {
    if ((0, import_fs_extra2.existsSync)(routesJsonPath)) {
      try {
        ...
        const routesJson = await import_fs_extra2.default.readJSON(routesJsonPath);
...
async function writeStaticFile(outputDir, file, path2, overrides, cleanUrls = false) {
  ...
  await import_fs_extra2.default.mkdirp(dirname(dest));
  if ("fsPath" in file) {
    if (resolve(file.fsPath) === resolve(dest)) {
      return;
    }
    try {
      return await import_fs_extra2.default.link(file.fsPath, dest);
    } catch (_) {
    }
  }
...
async function writeFunctionSymlink(outputDir, dest, fn, existingFunctions) {
  ...
  await import_fs_extra2.default.mkdirp(destDir);
  await import_fs_extra2.default.symlink(target, dest);
  return true;
}
async function writeContainerImage(outputDir, containerImage, path2, functionConfiguration) {
  ...
  await import_fs_extra2.default.mkdirp(dest);
...
async function writeEdgeFunction(repoRootPath, outputDir, edgeFunction, path2, existingFunctions, standalone = false) {
  ...
  ops.push(
    import_fs_extra2.default.writeJSON(configPath, config, {
      spaces: 2
    })
  );
...
async function writeLambda(repoRootPath, outputDir, lambda, path2, functionConfiguration, existingFunctions, standalone = false) {
  ...
  for await (const dir of findDirs(".vercel", dest)) {
    ...
    const entries = await import_fs_extra2.default.readdir(absDir);
    if (entries.includes("cache")) {
      await Promise.all(
        entries.filter((e) => e !== "cache").map((entry) => import_fs_extra2.default.remove(join2(absDir, entry)))
      );
    } else {
      await import_fs_extra2.default.remove(absDir);
    }
  }
}
...
async function cleanIgnoredFiles(dir, staticRoot, filter) {
  const entries = await import_fs_extra2.default.readdir(dir);
  await Promise.all(
    entries.map(async (entry) => {
      const entryPath = join2(dir, entry);
      const stat2 = await import_fs_extra2.default.stat(entryPath);
      const relativePath = relative2(staticRoot, entryPath);
      if (stat2.isDirectory()) {
        await cleanIgnoredFiles(entryPath, staticRoot, filter);
        const remaining = await import_fs_extra2.default.readdir(entryPath);
        if (remaining.length === 0) {
          await import_fs_extra2.default.rmdir(entryPath);
        }
      } else if (!filter(relativePath)) {
        output_manager_default.debug(`Removing ignored file: ${relativePath}`);
        await import_fs_extra2.default.remove(entryPath);
      }
    })
  );
}
...
async function* findDirs(name, dir, root = dir) {
  let paths;
  try {
    paths = await import_fs_extra2.default.readdir(dir);
  } catch (err) {
    if (err.code !== "ENOENT") {
      throw err;
    }
  ...
  for (const path2 of paths) {
    ...
    let stat2;
    try {
      stat2 = await import_fs_extra2.default.lstat(abs);
    } catch (err) {
      if (err.code === "ENOENT")
        continue;
      throw err;
...
var asAbsolute = function(path2, parent) {
  ...
  return resolve2(parent, path2);
};
async function staticFiles(path2, { src }) {
  const { debug, time } = output_manager_default;
  let files = [];
  const source = src || ".";
  const search = resolve2(path2, source);
  const { ig } = await (0, import_client2.getVercelIgnore)(path2);
  const filter = ig.createFilter();
...
async function explode(paths, { accepts }) {
  ...
  const list = async (file) => {
    ...
    try {
      s = await import_fs_extra3.default.stat(path2);
    } catch (_e) {
      path2 = `${file}.js`;
      try {
        s = await import_fs_extra3.default.stat(path2);
    ...
    if (s.isDirectory()) {
      let all;
      try {
        all = await import_fs_extra3.default.readdir(file);
      } catch (err) {
