...
function code(cmd, { backticks = true } = {}) {
  const tick = backticks ? import_chalk.default.gray("`") : "";
  return `${tick}${import_chalk.default.bold(cmd)}${tick}`;
}
...
var DomainNotFound = class extends NowError {
  constructor(domain, contextName) {
    super({
      ...
      meta: { domain },
      message: `Domain not found by "${domain}"${contextName ? ` under ${import_chalk2.default.bold(contextName)}` : ""}.`
    });
  }
};
